...
import torch
from model import LLcheM, SinusoidalPosEmbedding, TrainablePosEmbedding
...
def step(model, x, y, mask, padding, loss_fn, device, cross_entropy=False):
    ...
    if cross_entropy:
        # calculate cross entropy for estimating perplexity
        # TODO if loss_fn is cross entrpoy then computation is wasted here by duplicating steps, but loss_fn may be weighted.
        # preds contains only preds for masked elements.
        cross_entropy = F.cross_entropy(preds, y).item()
...
def train():
    ...
    if rank == 0 and Config.log_wandb_:
        wandb.init(
        project="LLcheM",
    ...
    train_loader, val_loader, vocab, tokens = get_dataloaders(Config.dataset_path_, use_lmdb=Config.lmdb_,
                                                      train_args=dict(num_workers=Config.num_workers_, batch_size=batch_size, pin_memory=Config.n_gpu > 0),
                                                      test_args=dict(num_workers=4, batch_size=Config.eval_batch_size), distributed=Config.n_gpu > 1)
    ...
    model = LLcheM(vocab, 
                  ...
                  pos_encoding_layer=None if Config.pos_layer == "rotary" else Config.pos_layer, max_input_len=1000,
                  rotary_embeddings=Config.pos_layer=="rotary").to(device)
    ...
    while True:
        ...
        with torch.no_grad(), ddp_no_sync():
            ...
            for batch_idx, batch in loader:
                with torch.autocast(device_type=device_type, enabled=autocast):
                    loss, ce = step(model, *batch, loss_fn, device, cross_entropy=True)
        ...
        if Config.n_gpu > 1:
            ...
            # average running cross entropy loss for perplexity calculation over all GPUs
            running_ce = torch.tensor(running_ce).to(rank)
            dist.all_reduce(running_ce, op=dist.ReduceOp.SUM)
            running_ce = running_ce.item() / Config.n_gpu
        perplexity = torch.exp(torch.tensor(running_ce) / batch_idx).item()
